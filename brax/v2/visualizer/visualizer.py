...
from brax.v2.base import Actuator, Convex, Mesh, Plane, Transform
from brax.v2.generalized import pipeline as generalized
from brax.v2.io import html
from brax.v2.io import mjcf
from brax.v2.positional import pipeline as positional
from brax.v2.spring import pipeline as spring
...
PORT = flags.DEFINE_integer(name="port", default=8080, help="Port to run server on")
DEBUG = flags.DEFINE_boolean(name="debug", default=False, help="Debug the server.")
...
def simulate(path):
    ...
    if force_floor:
        ...
        if floors:
            floor = floors[0]
            masks = [0 if is_mesh(g) else 1 if g is floor else 1 << 32 for g in sys.geoms]
    ...
    return html.render(sys, states, height="100vh", colab=False, base_url="/js/viewer.js")


def main(_):
    if DEBUG.value:
        flask_app.run(host="localhost", port=PORT.value, use_reloader=True, debug=True)
        return

    server = simple_server.make_server("localhost", PORT.value, validate.validator(flask_app))
